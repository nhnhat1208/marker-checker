...
_HELP_TEXT = (
    ...
    "Approver can be either an email address or a Telegram-style @handle.\n\n"
    "<b>Example messages:</b>\n"
    "• <code>enable feature-X on api-gateway, ask annie@example.com to approve</code>\n"
    "• <code>tắt tính năng beta trên nginx, nhờ manager@example.com duyệt</code>\n"
...
class TelegramCommandsMixin:
    ...
    def _register_user(self, update: Update) -> None:
        ...
        with self._registry_lock:
            if user.username:
                key = f"@{user.username}"
                if self._chat_registry.get(key) != chat_id:
                    self._chat_registry[key] = chat_id
                    new_pairs.append((key, chat_id))
            key2 = f"telegram:{user.id}"
            if self._chat_registry.get(key2) != chat_id:
                self._chat_registry[key2] = chat_id
                new_pairs.append((key2, chat_id))

        store = getattr(self, "_workflow_store", None)
        if store is not None and new_pairs:
            for handle, cid in new_pairs:
                threading.Thread(
                    target=store.upsert_chat_id, args=(handle, cid), daemon=True
                ).start()
