...
from agent.persistence.google_sheets_mapper import (
    AUDIT_HEADERS,
    CHAT_REGISTRY_HEADERS,
    CONVERSATION_HEADERS,
    PENDING_DRAFT_HEADERS,
    PENDING_RESUBMIT_HEADERS,
    REQUEST_HEADERS,
    audit_from_row,
    audit_to_row,
    conversation_to_row,
...
if TYPE_CHECKING:
    from agent.config import GoogleSheetsConfig
...
class GoogleSheetsWorkflowStore:
    ...
    def __init__(self, config: GoogleSheetsConfig) -> None:
        ...
        # Cache raw worksheet values for 15s; maxsize=8 to cover all worksheets.
        # Invalidated immediately after every write so reads post-write are always fresh.
        self._sheet_cache: TTLCache[str, list[list[str]]] = TTLCache(maxsize=8, ttl=15)
    ...
    def initialize(self) -> None:
        ...
        with self._lock:
            self._ensure_worksheet(self._config.worksheets.requests, REQUEST_HEADERS)
            self._ensure_worksheet(self._config.worksheets.audit_events, AUDIT_HEADERS)
            self._ensure_worksheet(
                self._config.worksheets.request_conversations, CONVERSATION_HEADERS
            )
            self._ensure_worksheet(
                self._config.worksheets.chat_registry, CHAT_REGISTRY_HEADERS
            )
            self._ensure_worksheet(
                self._config.worksheets.pending_drafts, PENDING_DRAFT_HEADERS
            )
            self._ensure_worksheet(
                self._config.worksheets.pending_resubmit, PENDING_RESUBMIT_HEADERS
            )
    ...
    def load_chat_registry(self) -> dict[str, str]:
        worksheet = self._chat_registry_worksheet()
        return {
            row["handle"]: row["chat_id"]
            for row in self._iter_rows(worksheet)
            if row.get("handle") and row.get("chat_id")
        }
    ...
    def upsert_chat_id(self, handle: str, chat_id: str) -> None:
        ...
        with self._lock:
            worksheet = self._chat_registry_worksheet()
            row_number = self._find_row_number(worksheet, "handle", handle)
            if row_number is not None:
                end_cell = rowcol_to_a1(row_number, len(CHAT_REGISTRY_HEADERS))
                worksheet.update([[handle, chat_id, updated_at]], f"A{row_number}:{end_cell}")
            else:
                worksheet.append_row(
                    [handle, chat_id, updated_at], value_input_option=ValueInputOption.raw
                )
            self._sheet_cache.pop(worksheet.title, None)
    ...
    def load_pending_drafts(self) -> list[dict[str, str]]:
        ...
        worksheet = self._pending_drafts_worksheet()
        return [
            row for row in self._iter_rows(worksheet)
            if row.get("handle") and row.get("expires_at", "") > now
        ]
    ...
    def upsert_pending_draft(self, handle: str, data: dict[str, str]) -> None:
        with self._lock:
            ...
            if row_number is not None:
                end_cell = rowcol_to_a1(row_number, len(PENDING_DRAFT_HEADERS))
                worksheet.update([row], f"A{row_number}:{end_cell}")
            else:
                worksheet.append_row(row, value_input_option=ValueInputOption.raw)
            self._sheet_cache.pop(worksheet.title, None)
    ...
    def load_pending_resubmit(self) -> list[dict[str, str]]:
        ...
        now = datetime.now(UTC).isoformat()
        worksheet = self._pending_resubmit_worksheet()
        return [
            row for row in self._iter_rows(worksheet)
            if row.get("handle") and row.get("expires_at", "") > now
        ]

    def upsert_pending_resubmit(self, handle: str, request_id: str, expires_at: str) -> None:
        with self._lock:
            worksheet = self._pending_resubmit_worksheet()
            row_number = self._find_row_number(worksheet, "handle", handle)
            row = [handle, request_id, expires_at]
            if row_number is not None:
                end_cell = rowcol_to_a1(row_number, len(PENDING_RESUBMIT_HEADERS))
                worksheet.update([row], f"A{row_number}:{end_cell}")
            else:
                worksheet.append_row(row, value_input_option=ValueInputOption.raw)
            self._sheet_cache.pop(worksheet.title, None)
    ...
    def _chat_registry_worksheet(self) -> gspread.Worksheet:
        return self._get_worksheet(self._config.worksheets.chat_registry, CHAT_REGISTRY_HEADERS)

    def _pending_drafts_worksheet(self) -> gspread.Worksheet:
        return self._get_worksheet(self._config.worksheets.pending_drafts, PENDING_DRAFT_HEADERS)

    def _pending_resubmit_worksheet(self) -> gspread.Worksheet:
        return self._get_worksheet(
            self._config.worksheets.pending_resubmit, PENDING_RESUBMIT_HEADERS
        )
