...
Rules: approver_handle may be an email address or an @handle when present. Unknown values must be empty strings.
...
- For new_request, also extract:
  ...
  - approver_handle: reviewer as an email or @handle (e.g. annie@example.com, @ops), empty string if not mentioned
...
- "tắt feature-X, nhờ annie@example.com duyệt" → {"operation":"new_request","request_id":"","target_name":"","note":"","text":"","target_label":"feature-X","change_from_summary":"enabled","change_to_summary":"disabled","approver_handle":"annie@example.com","guidance_message":""}
...
def build_request_parse_user_prompt(message: str, user_context: str | None = None) -> str:
    ...
    return (
        ...
        "Examples:\n"
        "- 'for nginx-config, change from 30s to 60s, ask annie@example.com to approve'\n"
        "- 'enable dark mode on dashboard, @designer should review'\n"
        "- 'I need annie@example.com to approve deploying v2.1 to prod'\n"
        "- 'tắt tính năng beta trên api-gateway, nhờ manager@example.com duyệt giúp'\n"
        f"{context_block}"
...
def build_intent_classify_user_prompt(message: str, user_context: str | None = None) -> str:
    context_block = f"\nKnown context about this user:\n{user_context}\n" if user_context else ""
    return f"Classify this message:{context_block}\nMessage: {message}"


REQUEST_IMPACT_NOTE_SYSTEM_PROMPT = """
Analyze a marker/flag change request and write one sentence describing the business impact.
Focus on magnitude or risk: percentage change, enable/disable of a live feature, config vs behaviour change.
If the impact is trivial or unclear, return an empty string.
Plain text only. No Markdown. One sentence, 25 words max.
""".strip()


def build_impact_note_user_prompt(*, target_label: str, change_from: str, change_to: str) -> str:
    return (
        f"Target: {target_label}\n"
        f"From: {change_from}\n"
        f"To: {change_to}\n"
        "Describe the business impact in one sentence, or return empty string if trivial."
    )
