...
from agent.parsing.request_parser import ParsedRequest, list_missing_required_fields
from agent.utils.approver_handles import (
    is_valid_approver_handle,
    normalize_approver_handle,
)
...
class RequestInputAssistant(Protocol):
    def classify_intent(self, text: str, user_context: str | None = None) -> IntentResult: ...
    ...
    def generate_impact_note(
        self, *, target_label: str, change_from: str, change_to: str
    ) -> str | None: ...
...
class OpenAICompatibleInputAssistant:
    ...
    def classify_intent(self, text: str, user_context: str | None = None) -> IntentResult:
        try:
            payload = self._client.complete_json(
                ...
                user_prompt=build_intent_classify_user_prompt(text, user_context),
    ...
    def assist_request_text(self, text: str, user_context: str | None = None) -> AssistedParseResult:
        ...
        try:
            payload = self._client.complete_json(
                ...
                user_prompt=build_request_parse_user_prompt(text, user_context),
                max_tokens=self._config.max_tokens,
            )
        except Exception as exc:
    ...
    def generate_impact_note(
        ...
        return self._generate_text(
            system_prompt=REQUEST_IMPACT_NOTE_SYSTEM_PROMPT,
            user_prompt=build_impact_note_user_prompt(
                target_label=target_label, change_from=change_from, change_to=change_to,
            ),
            max_tokens=self._config.max_tokens,
        )
    ...
    def _normalize_approver_handle(self, value: str) -> str:
        return normalize_approver_handle(value)
    ...
    def _validate_parsed_request(
        ...
        if approver_handle and not is_valid_approver_handle(approver_handle):
