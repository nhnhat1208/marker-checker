# Documentation Map

The docs are grouped by product, technical design, and delivery.

## Product Spec

Use these docs to understand product behavior and request workflow:

- [Overview](./product-spec/overview.md)
- [Workflow And Lifecycle](./product-spec/workflow-and-lifecycle.md)
- [Data Model And Audit](./product-spec/data-model-and-audit.md)

## Technical Design

Use these docs to understand implementation direction:

- [Scope And Channel Decision](./technical-design/scope-and-channel-decision.md)
- [Architecture](./technical-design/architecture.md)
- [Configuration And Integrations](./technical-design/configuration-and-integrations.md)
- [Diagrams](./technical-design/diagrams.md)

## Delivery Plan

Use these docs to prepare implementation and release:

- [Implementation Plan](./delivery-plan/implementation-plan.md)
- [AgentBase Execution Plan](./delivery-plan/agentbase-execution-plan.md)
- [Risks, Assumptions, And Open Questions](./delivery-plan/risks-assumptions-and-open-questions.md)

## Future Extensions

Read these only when the core approval workflow is already proven useful:

- [AI Review Assistance](./future/ai-review-assistance.md)
